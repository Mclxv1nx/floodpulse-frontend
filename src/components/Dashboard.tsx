  ...
  const load = useCallback(
    async (opts?: { silent?: boolean }) => {
      ...
      try {
        ...
        setError(msg);
        if (autoSource && source === 'backend' && /fetch|NetworkError|Failed/i.test(msg)) {
          setSource('mock');
        }
      } finally {
        clearInterval(tick);
        ...
        <section className="card">
          ...
          <div className="field">
            <div className="row">
              ...
              <b>{realRain ? 'satélite + pronóstico' : `${rain} mm`}</b>
            </div>
            <input
              type="range"
              min={0}
          ...
          <div className="row">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (v === 'auto') {
                  setAutoSource(true);
                  void backendAlive().then((ok) => setSource(ok ? 'backend' : 'mock'));
                } else {
                  setAutoSource(false);
